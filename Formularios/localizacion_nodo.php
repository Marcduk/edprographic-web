...
<head>
  ...
  <title>Localización de nodos</title>
...
<div class="container">
  ...
  if (isset($_POST['btn_consultar'])) {
    ...
    if ($id_loc_nod == "") {
      echo '<div class="alert alert-warning">Se requiere el número ID de la localización del nodo</div>';
    } else {
      $ver = $myPDO->query("SELECT * FROM localizacion_nodo WHERE id_localizacion_nodo = '$id_loc_nod'");
      if ($row = $ver->fetch()) {
        echo '<div class="alert alert-info">';
        echo 'ID: ' . $row['id_localizacion_nodo'] . '<br>';
        ...
        echo '<div class="alert alert-warning">El ID Localización nodo no existe en la tabla</div>';
      }
    }
  }
  ...
  if (isset($_POST['btn_nuevo'])) {
    ...
    if ($id_loc_nod == "" || $id_pa == "" || $id_ciud == "" || $direcc == "" || $cod_post == "") {
      echo '<div class="alert alert-warning">Los campos son obligatorios</div>';
    } else {
      $sql_insert = "INSERT INTO localizacion_nodo
                     (id_localizacion_nodo, id_pais, id_ciudad, direccion, codigo_postal)
                     VALUES ('$id_loc_nod', '$id_pa', '$id_ciud', '$direcc', '$cod_post')";
      $myPDO->query($sql_insert);
      echo '<div class="alert alert-success">La localización del nodo se registró en la tabla</div>';
    }
  }
  ...
  if (isset($_POST['btn_actualizar'])) {
    ...
    if ($id_loc_nod == "" || $id_pa == "" || $id_ciud == "" || $direcc == "" || $cod_post == "") {
      ...
      $ver = $myPDO->query("SELECT * FROM localizacion_nodo WHERE id_localizacion_nodo = '$id_loc_nod'");
      if ($row = $ver->fetch()) {
        $sql_update = "UPDATE localizacion_nodo 
                       SET id_pais = '$id_pa', 
                           id_ciudad = '$id_ciud', 
                           direccion = '$direcc', 
                           codigo_postal = '$cod_post' 
                       WHERE id_localizacion_nodo = '$id_loc_nod'";
  ...
  if (isset($_POST['btn_eliminar'])) {
    ...
    if ($id_loc_nod == "") {
      ...
      $ver = $myPDO->query("SELECT * FROM localizacion_nodo WHERE id_localizacion_nodo = '$id_loc_nod'");
      if ($row = $ver->fetch()) {
        $sql = "DELETE FROM localizacion_nodo WHERE id_localizacion_nodo = '$id_loc_nod'";
        $myPDO->query($sql);
